...
// --- UPDATED Asteroid Component with Better Mobile Clicking ---
const Asteroid = ({ lesson, index, userProgress, isMobile, onClick, onPointerOver, onPointerOut }) => {
    ...
    const { position, rotation, speed } = useMemo(() => {
        ...
        return {
        position: [radius * Math.cos(angle), (Math.random() - 0.5) * (isMobile ? 1.5 : 2), radius * Math.sin(angle)],
    ...
    // Much larger size on mobile for easier clicking
    const asteroidSize = isMobile ? 0.8 : 0.6;

    return (
        <group ref={pivotRef}>
        {/* Invisible larger hitbox for easier clicking on mobile */}
        {isMobile && !isLocked && (
          <mesh
            position={position}
        ...
        >
            ...
            />
            {/* Icons with pointer-events disabled */}
            {isLocked && (
            <Html center pointerEvents="none" style={{ pointerEvents: 'none' }}>
                <div style={{ pointerEvents: 'none' }}>
                <FaLock className={`text-white opacity-70 ${isMobile ? 'text-xl' : 'text-2xl'}`} style={{ pointerEvents: 'none' }} />
                </div>
            </Html>
            )}
            {isCompleted && (
            <Html center pointerEvents="none" style={{ pointerEvents: 'none' }}>
                <div style={{ pointerEvents: 'none' }}>
                <FaCheck className={`text-white ${isMobile ? 'text-xl' : 'text-2xl'}`} style={{ pointerEvents: 'none' }} />
                </div>
            </Html>
            )}
        </mesh>
        ...
        <header className="w-full max-w-7xl mx-auto flex justify-between items-center pointer-events-auto">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <div className="text-right">
              <p className="font-semibold text-xs sm:text-sm">{currentUser?.name || currentUser?.email || 'Player'}</p>
